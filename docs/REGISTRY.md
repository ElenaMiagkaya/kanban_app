...
ProfilePage → ProfileWidget → useProfile → getProfile → getProfileByUserId (withRetry)
                              ...
                              → UpdateName (nameSlot)
                              → ChangeEmail (emailActions)
                              → ChangePassword (passwordActions)
...
ProfileWidget / Sidebar
  → useProfile (enabled: auth ready)
      → getProfile → getProfileByUserId (withRetry)
...
```
ChangePassword
  → changePasswordSchema (Zod: new/confirm/current password)
  → useChangePassword.mutate({ newPassword, currentPassword })
      → signInWithEmail(userEmail, currentPassword)
      → updateAuthUser({ password: newPassword }) (withRetry)
